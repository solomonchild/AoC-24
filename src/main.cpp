...
using FuncAnsT = std::tuple<std::function<void(Part, const std::string&, std::string, std::string, bool)>, std::string, std::string>;
std::array Solutions= {
...
int main(int argc, char** argv)
{
	...
	auto solve_day = [](int day, int part, const std::string& input_dir)
	{
		auto [solve_func, ans1, ans2] = Solutions[day-1];
		LOG("\nSolving day %d", day);

		if (part != -1)
			solve_func(Part(part-1), input_dir, ans1, ans2, TIME);
		else
		{
			solve_func(Part::One, input_dir, ans1, ans2, TIME);
			solve_func(Part::Two, input_dir, ans1, ans2, TIME);
		}
	};
